...
class MultiStepCICMetricModule(Module):
    ...
    def compute(self, input_streams_dict:Dict[str,object]) -> Dict[str,object] :
        ...
        if (compute and np.random.random() < 0.2) or filtering_signal:
            ...
            logs_dict[f"{mode}/{self.id}/multi_step_CIC/{'Eval' if filtering_signal else 'Sample'}"] = ms_cic
            logs_dict[f"{mode}/{self.id}/UnconditionedPolicyFitting/CrossEntropyLoss/{'Eval' if filtering_signal else 'Sample'}"] = L_ce
            logs_dict[f"{mode}/{self.id}/UnconditionedPolicyFitting/PredictionAccuracy/{'Eval' if filtering_signal else 'Sample'}"] = prediction_accuracy
            
            if self.biasing:
                losses_dict = input_streams_dict["losses_dict"]
                losses_dict[f"{mode}/{self.id}/PositiveListeningLoss/{'Eval' if filtering_signal else 'Sample'}"] = [1.0, L_pl]
            else:
                logs_dict[f"{mode}/{self.id}/PositiveListeningLoss/{'Eval' if filtering_signal else 'Sample'}"] = L_pl
