...
# Shape Input:
# (seq_length, batch_size, state_dim)

class GatedTransformerXLBody(nn.Module):
    def __init__(
        self, 
        state_dim, 
        nbr_mem_slots=512,
        hidden_dim=256,
        nbr_heads_per_layer=8,
        head_output_dim=64,
        nbr_layers=12, 
        ...
        '''
        super(GatedTransformerXLBody, self).__init__()
        self.state_dim = state_dim
        self.hidden_dim = hidden_dim
        self.nbr_heads_per_layer = nbr_heads_per_layer
        self.head_output_dim = head_output_dim 
        self.nbr_layers = nbr_layers

        self.input_encoder = layer_init(
            nn.Linear(self.state_dim, self.hidden_dim, bias=False),
            init_type="ortho",
        )
