...
class ReadWriteHeads(BasicDNCHeads):
    ...
    def simplified_write(
        ...
        # Write weights:
        if timestep < self.memory.mem_nbr_slots:
            write_weights = torch.zeros(batch_size, 1, self.memory.mem_nbr_slots).index_fill_(
                dim=-1,
                index=torch.LongTensor([timestep]),
                value=1.0,
            ).to(memory_state.device)
        else:
            ...
            for bidx in range(batch_size)
                wr.append(
                    torch.zeros(1, 1, self.memory.mem_nbr_slots).index_fill_(
                        dim=-1,
                        index=least_used_index[bidx],
                        value=1.0,
                    ).to(memory_state.device)
                )
            write_weights = torch.cat(wr, dim=0)
            import ipdb; ipdb.set_trace()
 
        # Retroactive Adressing:
        ## Interpolation between prev_write_weights and prev_retroactive_weights:
        ret_write_weights = discount_factor*prev_ret_write_weights+(1-discount_factor)*write_weights
        #(batch_size x 1 x nbr_mem_slots  )
        ...
        return new_memory_state, new_write_weights, ret_write_weights
...
class DNCBody(nn.Module) :
    ...
    def forward(self, inputs):
        ...
        if self.simplified:
            ...
            read_vec, new_usage_vector = self.readWriteHeads.simplifiex_read(
            ...
            # Memory Write:
            written_memory_state, new_write_weights =self.readWriteHeads.simplified_write(
                memory_state=memory_state,
                odict=odict,
                prev_write_weights=prev_write_weights,
            )
