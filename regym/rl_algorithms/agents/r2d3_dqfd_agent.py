...
from .r2d3_agent import R2D3Agent
from ..algorithms.R2D3_DQfD import R2D3DQfDAlgorithm
...
def build_R2D3DQfD_Agent(task: 'regym.environments.Task',
                     config: Dict, 
                     agent_name: str):
    '''
    ...
    if kwargs['minerl']:
        ...
        if kwargs["expert_buffer_path"] != "":
            ...
            print(expert_agent.algorithm.model)
            expert_buffer = expert_agent.algorithm.storages[0]
        else:
            expert_buffer = None 
            
    algorithm = R2D3DQfDAlgorithm(
        kwargs=kwargs, 
        model=model,
