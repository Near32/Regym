...
--use_cuda=True \
--config=10bitsswap_benchmark_baseline_from_oracles_smallarch_ether_config.yaml \
...
--THER_feedbacks_failure_reward=-1 --THER_feedbacks_success_reward=0 \
...
--tau=None --inverted_tau=320 --learning_rate=1.0e-3 \
...
# INVERTED TAU = nbr_episode_per_cycle * T_{max} = 16 * 20 = 320
# Using inverted tau implies hard updates at repeated integer interval of data gathered, 
# whereas using tau implies soft update at each transition tupled being harvested.
