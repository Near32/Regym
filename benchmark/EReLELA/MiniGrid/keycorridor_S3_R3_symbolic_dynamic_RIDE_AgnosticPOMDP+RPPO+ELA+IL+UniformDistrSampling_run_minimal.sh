...
--use_cuda=True \
--project=EReLELA+IL+RPPO-KeyCorridor-Symbolic-Benchmark \
--success_threshold=0.01 \
--config=keycorridor_S3_R3_symbolic_minigrid_wandb_RIDE_benchmark_AgnosticPOMDPERELELA+RPPO_config.yaml \
...
--ELA_reward_extrinsic_weight=1.0 --ELA_reward_intrinsic_weight=0.01 \
...
--nbr_minibatches=1 --batch_size=512 \
--min_capacity=4e3 --min_handled_experiences=48e3 --replay_capacity=20e3 \
--learning_rate=3.0e-4 \
--sequence_replay_burn_in_ratio=0.0 --weights_entropy_lambda=0.0 \
--sequence_replay_unroll_length=8 --sequence_replay_overlap_length=0 \
...
--train_observation_budget=20.0e6
