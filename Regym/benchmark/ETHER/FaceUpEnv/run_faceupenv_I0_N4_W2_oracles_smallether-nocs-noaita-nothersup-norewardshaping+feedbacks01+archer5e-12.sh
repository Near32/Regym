...
--use_cuda=True \
--config=FaceUpEnv_I0_N4_W2_wandb_benchmark_MultiplicativeSemanticPrior_OracleSmallETHER_config.yaml \
...
--ETHER_rg_semantic_cooccurrence_grounding_sentence_level=False \
...
--ETHER_rg_gaussian_blur_prob=0.0 \
...
--ETHER_rg_vocab_size=64 --ETHER_rg_training_period=256000 \
...
--nbr_training_iteration_per_cycle=128 --nbr_episode_per_cycle=8 \
--single_pick_episode=False --THER_timing_out_episode_length_threshold=20 \
--time_limit=20 \
--train_observation_budget=2000000

# The environment already provides achieved goal info, thus there is no need for THER_observe_achieved_goal.
# But, it is necessary to set FaceUpObject_oracle=True.
